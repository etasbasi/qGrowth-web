...
function Dashboard() {
  ...
  async function addTree(index) {
    let x = randomNumber(0, gardenRef.current.clientWidth - 50);
    let y = randomNumber(0, gardenRef.current.clientHeight - 180);
    ...
    if (filterTreesByLevel.length === 0) {
      ...
      try {
        ...
        tempTrees[getTreeIndex(treeToUpdate.id)] = treeToUpdate;
  ...
  if (loading)
    return <Dots style={{ height: "100px", width: "100px", margin: "auto" }} />;

  return (
    <div className="dashboard">
      <div className="list">
        {items.map((item, index) => (
          <div onClick={() => addTree(index)} key={index} className="list-item">
            <p>{item.name}</p>
      ...
      <Button className="delete-button" onClick={deleteAll} isDanger>
        Remove all trees
      </Button>
    </div>
  );
